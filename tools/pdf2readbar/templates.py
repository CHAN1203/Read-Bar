...
def hub_page(book_id, title, subtitle, accent, cards, spine_js):
    return (
        "<!DOCTYPE html><html lang='zh-CN'><head><meta charset='UTF-8'>"
        "<meta name='viewport' content='width=device-width, initial-scale=1.0'>"
        f"<title>{title}</title></head>"
        f"<body style='background:#13161b;color:#e9e5d8;font-family:Georgia,serif;"
        f"max-width:800px;margin:0 auto;padding:40px'>"
        f"<a href='../../index.html'>← 书架</a><h1>{title}</h1><p>{subtitle}</p>"
        f"{cards}</body></html>"
    )
